		...
		foreach($fields as $key => $feild){
			...
			if($status){
				...
				$data['feilds'][$key]=array(
					...
					'label'			=> ($onepagecheckout_manage['register'][$key]['label'][$this->config->get('config_language_id')] ? $onepagecheckout_manage['register'][$key]['label'][$this->config->get('config_language_id')] : $this->language->get('entry_'.$key)),
