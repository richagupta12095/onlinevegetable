...
class ControllerAccountAddress extends Controller {
	...
	public function updateaddress() {
		...
		if (!$this->customer->isLogged()) {
	 	...
	 	if(empty($address_id)){
	 		$json['error']=true;
			$json['msg']='Add configuration is missing!';
	 	...
	 	$data=array(
			'firstname'=>isset($_POST['edit_firstname'])?$_POST['edit_firstname']:'',
	 		...
	 		'address_1'=>isset($_POST['edit_address_1'])?$_POST['edit_address_1']:'',
	 		'address_2'=>isset($_POST['edit_firstname'])?$_POST['edit_firstname']:'',
	 		'company='=>'',
	 		'postcode'=>isset($_POST['edit_postcode'])?$_POST['edit_postcode']:'',
	 		'country_id'=>isset($_POST['edit_country_id'])?$_POST['edit_country_id']:'',
	 		'zone_id'=>isset($_POST['edit_zone_id'])?$_POST['edit_zone_id']:'',
	 		'city'=>isset($_POST['edit_city_val'])?$_POST['edit_city_val']:'',
	 	);
		error_reporting(0);
	 	$res=$this->model_account_address->editAddress($address_id,$data);
	 	$address=$this->model_account_address->getAddresses();

	 	if(!empty($address)){
	 		$options='';
	 		foreach ($address as $key => $value) {
	 			$opt=$value['firstname']." ".$value['lastname']." ,".$value['address_1']." ,".$value['address_2']." ,".$value['city']." ,".$value['zone']." ,".$value['country'];
	 			...
	 			$options .="<option ".$sel." value=".$value['address_id'].">".$opt."</option>";
	 		}

	 	}
	 	$json['error']=true;
 		$json['address_id']=$_POST['address_id'];
 		$json['addressdata']=$options;
	...
	protected function getForm() {
		...
		if(!empty($data['zone_id'])){
			...
			$data['cities'] = $this->model_localisation_zone->getCitiesByZoneId($data['zone_id']);
		}else{
			$data['cities'] = array();
		}
