	...
	public function addCustomerOrder() {
		...
		if (isset($this->request->post['customer_id'])) {
			...
			if (1) { 
				...
				// Customer Details
				$order_data['customer_id'] 		= 	$this->request->post['customer']['customer_id'];
				$order_data['customer_group_id'] = 	$this->request->post['customer']['customer_group_id'];
				$order_data['firstname'] 		= 	$this->request->post['customer']['firstname'];
				$order_data['lastname'] 		= 	$this->request->post['customer']['lastname'];
				$order_data['email'] 			= 	$this->request->post['customer']['email'];
				$order_data['telephone'] 		= 	$this->request->post['customer']['telephone'];
				$order_data['custom_field'] 	= 	'{"1":"0000"}';

				// Payment Details
				$order_data['payment_firstname'] 	= $this->request->post['payment_address']['firstname'];
				$order_data['payment_lastname'] 	= $this->request->post['payment_address']['lastname'];
				$order_data['payment_company'] 		= '';
				$order_data['payment_address_1'] 	= $this->request->post['payment_address']['address_1'];
				$order_data['payment_address_2'] 	= $this->request->post['payment_address']['address_2'];
				$order_data['payment_city'] 		= $this->request->post['payment_address']['city'];
				$order_data['payment_postcode'] 	= $this->request->post['payment_address']['postcode'];
				$order_data['payment_zone'] 		= $this->request->post['payment_address']['zone'];
				$order_data['payment_zone_id'] 		= $this->request->post['payment_address']['zone_id'];
				...
				$totals = array();
				$taxes  = $this->getTaxes($order_data['products']);
				$total  = 0;
					...
					if ($this->config->get('total_' . $result['code'] . '_status')) {
						...
						if($result['code']=='sub_total'){ 
							$total_data['totals'][] = array(
												...
												'value'      => $this->getSubTotal($order_data['products']),
												'sort_order' => $this->config->get('sub_total_sort_order')
											);
							$total_data['total'] +=   $this->getSubTotal($order_data['products']);

						}else if($result['code']=='total'){							
							$total_data['totals'][] = array(
												'code'       => 'total',
												'title'      => $this->language->get('text_total'),
												'value'      => $this->getTotal($order_data['products']),
