...
class ModelApiEwallet extends Model {
	...
	public function gettransaction($data = array()) {
		...
		if (isset($data['datefrom'])) {
			$str .= " AND date_added >= '" . date('Y-m-d', strtotime($data['datefrom'])) . "'";
		}

		if (isset($data['dateto'])) {
			$str .= " AND date_added <= '" . date('Y-m-d', strtotime($data['dateto'] . ' +1 days')) . "'";
		}
	...
	public function getopenningbalance($data = array()) {
		...
		$str = "SELECT balance FROM `{$this->per}e_wallet_transaction`
			WHERE customer_id = " . (int) $this->customer->getId();
		if (isset($data['datefrom'])) {
			$str .= " AND DATE(date_added) <= '" . date('Y-m-d', $time) . "'";
		}
	...
	public function setbank($data = array()) {
		...
		";
		if ($bank) {
			$str .= " WHERE customer_id = " . (int) $customer_id;
		} else {
	...
	public function sendmoney($data = array()) {
		if (isset($data['customer_id']) && (int) $data['customer_id'] != 0) {
			...
			$str = "INSERT INTO `{$this->per}e_wallet_transaction` SET
				`customer_id` = '" . (int) $data['customer_id'] . "',
				`price` = '" . (float) $data['amount'] . "',
				`description` = '" . $this->db->escape('You Receive Money From ' . $data['sender_name']) . "',
				`date_added` = NOW()";
			$this->db->query($str);
			$transaction_id = $this->db->getLastId();
			...
			$str = "INSERT INTO `{$this->per}e_wallet_transaction` SET
				`customer_id` = '" . (int) $data['sender_customer_id'] . "',
				`price` = '" . (float)  - $data['amount'] . "',
			...
			$this->setbalance(array('customer_id' => $data['sender_customer_id'], 'transaction_id' => $transaction_id));
			...
			//Mail to Receiver
			$subject = $this->config->get('e_wallet_title') . " Money Transaction.";
			$message = 'You Receive Money From ' . $data['sender_name'] .
			" \n Amount : " . $data['amount'] .
			...
			//Mail to Money Sender
			$subject = $this->config->get('e_wallet_title') . " Money Transaction.";
			$message = 'You Send Money to ' . $data['name'] .
			" \n Amount : " . $data['amount'] .
			" \n Email : " . $data['email'] .
			" \n Date : " . date('d-m-Y h:i:s A');
			$mail->setTo($data['sender_email']);
			$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
			$mail->setText($message);
			$mail->send();
		}
	}
	public function orderAmountDeducted($data = array()) {

		$str = "INSERT INTO `{$this->per}e_wallet_transaction` SET
				`customer_id` = '" . (int) $data['customer_id'] . "',
				`price` = '" . (float)  - $data['amount'] . "',
				`description` = 'Payment Against Order #" . $data['order_id']." ".$this->db->escape(' Amount :' . $data['amount']) . "',
				`date_added` = NOW()";
		$this->db->query($str);
		$transaction_id = $this->db->getLastId();
		$this->setbalance(array('customer_id' => $data['customer_id'], 'transaction_id' => $transaction_id));		
