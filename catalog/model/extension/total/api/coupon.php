<?php
class ModelExtensionTotalApiCoupon extends Model {
	public function getCoupon($code, $data) {
		...
		if ($coupon_query->num_rows) {
			...
			if(isset($data['total']) && !empty($data['total'])){
				if ($coupon_query->row['total'] > $data['total']) {
					$status = false;
					$reason = 1;
					$min_total = $coupon_query->row['total'];
				}
			}else{
				if ($coupon_query->row['total'] > $this->model_extension_total_api_general->getSubTotal($products)) {
					$status = false;
					$reason = 1;
					$min_total = $coupon_query->row['total'];
				}
			...
			if ($coupon_query->row['logged'] && !$data['customer_id']) {
				$status = false;
				$reason = 3;
			}

			if ($data['customer_id']) {
				$customer_total = $this->getTotalCouponHistoriesByCustomerId($code, $data['customer_id']);
		...
		if ($status) {
			...
			switch ($reason) {
			case 1:
				$message = "Cart Value Should be grater than or equals to " . $min_total;
				break;
			case 2:
				$message = "You have used the coupon maximum time!";
				break;
			case 3:
				$message = "You are not allowded to use the Coupon.";
				break;
			case 4:
				$message = "You are not allowded to use the Coupon.";
				break;
			case 5:
				$message = "These Products are not allowded for the Coupon Code.";
				break;
			default:
				$message = "Coupon has expired or invalid.";
			}
			return array(
				'error' => 1,
				'reason' => $message,
	...
	public function getTotal($total, $data) {

		if (isset($data['coupon'])) {
			$this->load->language('extension/total/coupon', 'coupon');
			$products = $data['products'];
			$coupon_info = $this->getCoupon($data['coupon'], $data);
			...
			if ($coupon_info) {
				...
				*/
				...
				if ($discount_total > 0) {
					$total['totals'][] = array(
						'code' => 'coupon',
						'title' => sprintf($this->language->get('coupon')->get('text_coupon'), $data['coupon']),
