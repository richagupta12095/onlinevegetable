...
class ModelApiOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if(!empty($product['mpplan_id']) && $this->config->get('mpplan_status')) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "mpplan_customer` SET `order_id` = '" . (int)$order_id . "', `order_product_id` = '" . (int)$order_product_id . "', `customer_id` = '" . (int)$data['customer_id'] . "', `mpplan_id` = '" . (int)$product['mpplan_id'] . "', `plan_name` = '" . $this->db->escape($product['plan_name']) . "', `start_date` = '" . $this->db->escape($product['start_date']) . "', `end_date` = '" . $this->db->escape($product['end_date']) . "', `duration_type` = '" . $this->db->escape($product['duration_type']) . "', `duration_value` = '" . $this->db->escape($product['duration_value']) . "', `plan_info` = '" . $this->db->escape($product['plan_info']) . "', `active`=1 ");
				}
				/* MP Membership Ends */
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false,$mplan=0) {
	...
	public function updateMembershipDetails($data){
		/* MP Membership Starts */
			if(!empty($product['mpplan_details']) && $this->config->get('mpplan_status')) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "mpplan_customer` SET 
					`order_id` = '" . (int)$order_id . "', 
					`order_product_id` = '" . (int)$order_product_id . "', 
					`customer_id` = '" . (int)$data['customer_id'] . "', 
					`mpplan_id` = '" . (int)$product['mpplan_details']['mpplan_id'] . "', 
					`plan_name` = '" . $this->db->escape($product['mpplan_details']['plan_name']) . "', `start_date` = '" . $this->db->escape($product['mpplan_details']['start_date']) . "', 
					`end_date` = '" . $this->db->escape($product['mpplan_details']['end_date']) . "', `duration_type` = '" . $this->db->escape($product['mpplan_details']['duration_type']) . "', 
					`duration_value` = '" . $this->db->escape($product['mpplan_details']['duration_value']) . "', 
					`plan_info` = '" . $this->db->escape($product['mpplan_details']['plan_info']) . "'");
			}
		/* MP Membership Ends */
	}
