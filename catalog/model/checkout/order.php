...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (($order_info['order_status_id'] != $order_status_id) && $order_status_id == 5) {
				//$order_info['order_status_id']
				//Order Cashback Code By Ankur Mittal
				if (isset($order_info['total'])) {

					$cashback_check_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_cashback` ocb WHERE ocb.status = 1 AND ocb.min_amount <=" . $order_info['total'] . " AND ocb.max_amount >=" . $order_info['total']);

					$cashback_history_check_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_cashback_history` ocbh WHERE ocbh.order_id = '" . $order_info['order_id'] . "'");

					if ($cashback_check_query->num_rows > 0 && $cashback_history_check_query->num_rows == 0) {

						$this->load->model('api/e_wallet');

						$cashback_result = $cashback_check_query->row;

						if ($cashback_result['cashback_type'] == 'P') {
							$cashback_amount = ($order_info['total'] * $cashback_result['cashback_value']) / 100;
						} else {
							$cashback_amount = $cashback_result['cashback_value'];
						}

						$cashdata = array(
							'desc' => 'Cashbach Credited in Wallet with reference Order id #' . $order_id,
							'customer_id' => $order_info['customer_id'],
							'customer_name' => $order_info['firstname'] . ' ' . $order_info['lastname'],
							'customer_email' => $order_info['email'],
							'message' => 'Add Money In Wallet..!',
							'amount' => round($cashback_amount),
						);
						$this->model_api_e_wallet->addtransaction($cashdata);
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_cashback_history SET order_id = '" . (int) $order_id . "', customer_id = '" . $order_info['customer_id'] . "', `order_total` = '" . $order_info['total'] . "', `cashback_amount` = '" . ceil($cashback_amount) . "', `created_at` = NOW() ");
					}
				}
			}
	...
	//By Ankur Mittal
	public function addOrderVegOrderMapping($order_id,$veg_order_id) {
		if(!empty($order_id) && !empty($veg_order_id)){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "order_veg_order_mapping` SET order_id=".$order_id.", veg_order_id=".$veg_order_id.", date_added = NOW(), date_modified = NOW()");
		}
		return true;
	}
