...
class ControllerextensionOnepagecheckoutValidate extends Controller {
	...
	public function validateForm(){
		...
		if(!$this->customer->isLogged()){
			...
			if(!empty($this->request->post['account_type']) && ($this->request->post['account_type'] == 'register' || $this->request->post['account_type'] == 'guest')){
				...
				$captcha_status = (!empty($onepagecheckout_manage['personaldetails']['captcha_status']) ? $onepagecheckout_manage['personaldetails']['captcha_status'] : '');
				if ($this->config->get('captcha_' . $this->config->get('config_captcha') . '_status') && $captcha_status){
					$captcha = $this->load->controller('extension/captcha/' . $this->config->get('config_captcha') . '/validate');

					if ($captcha) {
						$json['error']['personal_details']['captcha'] = $captcha;
					}
				}
			...
			if(empty($this->request->post['delivery_date']) && $this->config->get('onepagecheckout_delivery_required') && $this->config->get('onepagecheckout_delivery_status')){
			...
			if(isset($this->request->post['payment_details'])){
				...
				if($this->request->post['payment_details']['payment_address']=='new'){
					...
					if(!$json){
						...
						if(!empty($this->request->post['payment_details']['shipping_address'])){
							$this->session->data['shipping_address'] = $this->model_account_address->getAddress($address_id);
						}
			...
			if ($this->cart->hasShipping()){
				if(isset($this->request->post['delivery_details']) && !isset($this->request->post['payment_details']['shipping_address'])){
					$this->setDeliveryRequiredFeilds();
			...
			$this->session->data['delivery_date'] = (isset($this->request->post['delivery_date']) ? $this->request->post['delivery_date'] : '');
			//By Ankur Mittal
			$this->session->data['slot-selection'] = (isset($this->request->post['slot-selection']) ? $this->request->post['slot-selection'] : '');
	...
	public function save($data,$customer_group_id){
		$this->load->model('extension/onepagecheckout/customer');
		$json=array();
	...
	public function savedata(){
		$json=array();
		if(isset($this->request->post['personal_details'])){
			foreach($this->request->post['personal_details'] as $key => $value){
				if($key=='customer_group_id'){
					$this->session->data['guest']['customer_group_id'] = $value;
				}
				
				if($key=='firstname'){
					$this->session->data['guest']['firstname'] = $value;
				}
				
				if($key=='lastname'){
					$this->session->data['guest']['lastname'] = $value;
				}
				
				if($key=='email' && filter_var($value, FILTER_VALIDATE_EMAIL)){
					$this->session->data['guest']['email'] = $value;
				}
				
				if($key=='telephone'){
					$this->session->data['guest']['telephone'] = $value;
				}
				
				if($key=='fax'){
					$this->session->data['guest']['fax'] = $value;
				}
			}
			$json[] = $this->session->data['guest']; 
		}
