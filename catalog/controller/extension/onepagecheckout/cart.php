...
class ControllerExtensionOnepagecheckoutCart extends Controller {
		...
		if(!empty($onepagecheckout_manage['shopping_cart']['shopping_cart_status'])) {
			...
			foreach ($products as $product) {
				if(!empty($product['categories'])){
					$search_key_for_cat1 = array_search('65', array_column($product['categories'], 'category_id'));
					$search_key_for_cat2 = array_search('66', array_column($product['categories'], 'category_id'));
					
					if(is_numeric($search_key_for_cat1)){
						$take_slot_value = 1;
					}else if(is_numeric($search_key_for_cat2)){
						$take_slot_value = 1;
					}
				}
				$product_total = 0;
			...
			foreach ($totals as $total) {
				...
				if($total['title']=='Total'){
					$prod_total = $total['value'];
				}
			}
			$veg_order_details = '';
			if(!empty($take_slot_value)){
				$this->load->model('account/membership');	
				$veg_order_details = $this->model_account_membership->getcurrentVegOrderDetails($this->session->data['customer_id']);	
			}
			$data['prod_total'] = $prod_total;		
			$data['total_save'] = $total_mrp - $prod_total;				
			$data['veg_order_details'] = $veg_order_details;	
	...
	public function edit() {
		...
		// Update
		if (isset($this->request->post['key']) && !empty($this->request->post['quantity'])) {
