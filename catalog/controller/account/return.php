...
class ControllerAccountReturn extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			//By Ankur Mittal
			$newfilename = '';
			if (is_uploaded_file($this->request->files['images']['tmp_name'])) {
			    $handle = fopen($this->request->files['images']['tmp_name'], "r");
			    
			    $temp = explode(".", $_FILES["images"]["name"]);
				$newfilename = round(microtime(true)) . '.' . end($temp);
			   //echp $newfilename;exit;
			   $uploads_dir = DIR_IMAGE."/return/";
			   move_uploaded_file($this->request->files['images']['tmp_name'],$uploads_dir.$newfilename);
 				$data['photo'] = $newfilename;
			    fclose($handle);
			} else {
			    $data['photo'] = array();
			}			
			$this->request->post['image_name'] = $newfilename;
