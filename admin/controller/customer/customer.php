...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['newsletter'])) {
			...
			$data['newsletter'] = $customer_info['ngetFormewsletter'];
		...
		//by Ankur Mittal
		$data['customer_membership'] = $this->model_customer_customer->getActivePlan($this->request->get['customer_id']);
		$data['customer_membership_addon_history'] = $this->model_customer_customer->getCustomerAddonHistory($this->request->get['customer_id']);
		
		// By Gagan
		$data['customer_membership_history'] = $this->model_customer_customer->getMembershipHistory($this->request->get['customer_id']);
		$this->load->model('customer/place_order');
		$this->load->language('account/membership');
		$this->load->model('account/membership');
		$this->load->model('membership/subscriptions');

		$cid=$this->request->get['customer_id'];
		$userdsetting=$this->model_account_membership->getdsetting($cid);
		$ar=[];
		...
		$count=$this->model_account_membership->getselectedday($cid);

		$monday = date('Y-m-d', strtotime( 'monday this week' ) );
		$customerExist  = array();
		$customerExist['week_date'] = $monday;	
		$customerExist['customer_id'] = $cid;
		$weekorder = $this->model_account_membership->checkweekorderexsit($customerExist);
		

	 	$mebsip=$this->model_account_membership->getMemebership($cid);
		...
		foreach ($results as $result) {
				...
				$dresult=$this->model_account_membership->getlistdelivery($cid);
		...
		// ends
		$dproudct='';
		$dproudct .='<option value="">Select Product</option>';
		if(!empty($data['products'])){
			foreach ($data['products'] as $key => $value) {
				# code...
				$dproudct .='<option value="'.$value['product_id'].'">'.$value['name'].'</option>';
			}

		}
		$data['setting']=$dsettings;
		$data['udsetting']=$userdsetting;
		$data['misAllow']=$isAllow;

		$data['timeslot'] = $timeslot;
		$data['dsetting'] = $dsetting;
		$data['totalweight'] = $totalweight;
		$data['dproudct']=$dproudct;
	...
	public function saveMembershipproduct(){

		echo '<pre>';
		print_r(json_decode($_POST['json_arr']));
		echo '<br/>';
		print_r(json_decode($_POST['selected_day']));
		die;
	...
	public function getProductweight(){
		...
		if($pid){
			$wresult=$this->model_account_membership->getProductweight($pid);
			// ends
			$dproudct='';
			$dproudct .='<option value="">Select Weight</option>';
			if(!empty($wresult)){
				foreach ($wresult as $key => $value) {
					# code...
					$dproudct .='<option value="'.$value['id'].'">'.$value['value'].'</option>';
				}
	...
	public function addOrder($data) {
		$userId = $this->request->get['customer_id'];
		if (!isset($userId) ) {
		...
		if (isset($_POST)) {
			$dsettings=$this->model_customer_place_order->deldsettings($this->request->get['customer_id']);
		 
			foreach ($_POST['dday'] as $key => $value) {
				$res=$this->model_customer_place_order->saveliverysetting($this->request->get['customer_id'],$value);
			}
			$json['error']='success';
			$json['redirect']=$this->url->link('account/membership', '', true);
		}

	 	$mebsip=$this->model_customer_place_order->getMemebership($this->request->get['customer_id']);
	 	if(empty($mebsip)){
 			$this->response->redirect($this->url->link('customer/customer', '', true));
	 	...
	 	if(!$mebsipId){
	 		...
	 		$mcat=$this->model_customer_place_order->getMemebershipprdcat($mebsipId);
	 		$parra=array();
	 		foreach ($mcat as $key => $value) {
	 				$parra[]=$value['category_id'];
	 			}	
		...
		$results=$this->model_customer_place_order->getProducts($filter_data);
		$url='';
		foreach ($results as $result) {
					$images[] = array(
						'popup' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_height')),
				...
				$insp_data['insp_images'][] = $result['image'];

				
				//Weight data


				$wresult=$this->model_customer_place_order->getWeightValuesByWeightId($result['weight_id']);
				$dresult=$this->model_customer_place_order->getlistdelivery($this->request->get['customer_id']);
		...
		$totalweight=$this->model_customer_place_order->getCustomerWeightAllowded($userId);
	
		$timeslot=	$count=$this->model_customer_place_order->getTimeslot();

		$dsetting=	$count=$this->model_customer_place_order->getlistdelivery($userId);
		$data['timeslot'] = $timeslot;
		$data['dsetting'] = $dsetting;
		$data['totalweight'] = $totalweight;
		$data['kgweight'] =number_format($totalweight/1000,2);
		...
		if($weekorder){
			...
			$results = $this->model_customer_place_order->getcurrentVegOrderDetails($userId);
			...
			$data['error']='data_issue';
			$this->response->setOutput($this->load->view('membership/my_membership_order', json_encode($data)));
		}else{
			$data['error']='success';
			$this->response->addHeader('Content-Type: application/json');

			$this->response->setOutput(json_encode($data));
			//$this->response->setOutput($this->load->view('customer/customer', json_encode($data)));
		}
	...
	public function export($getarr='') {
		...
		fputcsv($fp,  $customers_column);
		foreach($customers as $customer_row)
		{
			fputcsv($fp, $customer_row);
		}
	 
	 }
	//By Ankur Mittal	 
	 public function vegOrdersList() {

		$this->load->model('customer/customer');
		$this->load->language('customer/veg_orders');
		...
		$customer_total = $this->model_customer_customer->getCustomerVegOrdersTotal($filter_data);

		$results = $this->model_customer_customer->getCustomerVegOrders($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'week_date' => date($this->language->get('date_format_short'), strtotime($result['week_date'])),

				//'customer_group' => $result['customer_group'],

				'date_added' => date($this->language->get('date_format_short'), strtotime($result['created_at'])),

				'edit' => $this->url->link('customer/customer/vegOrderDetails', 'user_token=' . $this->session->data['user_token'] . '&id=' . $result['id'] . $url, true),
		...
		$pagination->url = $this->url->link('customer/customer/vegOrdersList', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('customer/customer_veg_orders_list', $data));

	}

	public function vegOrderDetails() {
		...
		$results = $this->model_customer_customer->getVegOrderDetails($this->request->get['id']);
		//print_r($results);exit;
		foreach ($results as $result) {
		...
		$data['cancel'] = $this->url->link('customer/customer/vegOrdersList', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('customer/veg_orders_detail', $data));
