...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
		...
		$data['reset'] = $this->url->link('catalog/category/resetDiscount', 'user_token=' . $this->session->data['user_token'] . '&category_id='.$this->request->get['category_id'].$url, true);
		...
		if (!empty($data['category_description'])) {
			$data['membership_assigned'] = $data['category_description'][1]['membership_assigned'];
		}
		...
		if (isset($this->request->post['discount_expiry'])) {

			$data['discount_expiry'] = $this->request->post['discount_expiry'];

		} elseif (!empty($category_info)) {

			$data['discount_expiry'] = $category_info['discount_expiry'];
	...
	public function resetDiscount() {
		if (isset($this->request->get['category_id']) && !empty($this->request->get['category_id'])) {
			$this->load->model('catalog/category');
			$this->model_catalog_category->resetDiscount($this->request->get['category_id']);
		}
		$this->response->redirect($this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . $url, true));
	}
