<?php
class ControllerExtensionOrderSuccessPage extends Controller {	
		private $error = array();
		private $ssl = 'SSL';
		private $tpl = '.tpl';
	...
	public function index() {
		$this->load->language('extension/checkout/order_success_page');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->session->data['success'] = $this->language->get('text_success');
				...
				$this->response->redirect($this->url->link('extension/order_success_page', '&store_id='.$data['store_id'].'&user_token=' . $this->session->data['user_token'] , $this->ssl));
		...
		$data['action'] = $this->url->link('extension/order_success_page', 'user_token=' . $this->session->data['user_token'] . '&store_id='. $data['store_id'], $this->ssl);
		
		$data['store_action'] =  $this->url->link('extension/order_success_page','user_token=' . $this->session->data['user_token'], $this->ssl . '&type=module');

		$data['cancel'] = $this->url->link('extension/onepagecheckout', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/checkout/order_success_page', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/order_success_page')) {
