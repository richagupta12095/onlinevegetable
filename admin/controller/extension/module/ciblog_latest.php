<?php
class ControllerExtensionModuleCiBlogLatest extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/module/ciblog_latest');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->{$model_extension_module}->addModule('ciblog_latest', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/ciblog_latest', $this->ciblog->octoken.'=' . $this->session->data[$this->ciblog->octoken], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/ciblog_latest', $this->ciblog->octoken.'=' . $this->session->data[$this->ciblog->octoken] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/ciblog_latest', $this->ciblog->octoken.'=' . $this->session->data[$this->ciblog->octoken], true);
		} else {
			$data['action'] = $this->url->link('extension/module/ciblog_latest', $this->ciblog->octoken.'=' . $this->session->data[$this->ciblog->octoken] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->response->setOutput($this->ciblog->view('extension/module/ciblog_latest', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/ciblog_latest')) {
