...
class ModelCustomerCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		...
		if ($data['affiliate']) {
			...
			//By Ankur Mittal
			if (!empty($data['addonamount']) && !empty($data['addonweight'])) {
				$current_active_plan = $this->getActivePlan($customer_id);
				if (!empty($current_active_plan)) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "mpplan_customer_addon SET
						customer_id = '" . (int) $customer_id . "',
						mpplan_id = '" . $current_active_plan['mpplan_id'] . "',
						plan_name = '" . $current_active_plan['plan_name'] . "',
						amount_charged = '" . $this->db->escape($data['addonamount']) . "',
						weight = '" . $this->db->escape($data['addonweight']) . "',
						comments = '" . $this->db->escape($data['addoncomments']) . "',
						created_at = NOW()");
	
					$this->db->query("UPDATE " . DB_PREFIX . "mpplan_customer SET weight_allowed = " . $current_active_plan['weight_allowed'] . " + " . $data['addonweight'] . "  WHERE customer_id = '" . $customer_id . "' AND active = 1 AND mpplan_id = '" . $current_active_plan['mpplan_id'] . "'");
	...
	public function getCustomerstoexport($data = array()) {
		...
		public function getCustomerAddonHistory($customer_id) {
			$sql = "SELECT mp.* FROM " . DB_PREFIX . "mpplan_customer_addon mp WHERE mp.customer_id = '" . (int) $customer_id . "' ORDER BY created_at DESC";
	...
	public function getCustomerVegOrders($customer_id) {
		$sql = "SELECT cwo.*,c.firstname,c.email,mc.plan_name,ds.name as delivery_day,ts.name as slot_name FROM `oc_customer_weekly_veg_orders` cwo
				...
				LEFT JOIN `oc_timeslots_settings` ts ON ts.id = cwo.slot_id
				";
	...
	public function getCustomerVegOrdersTotal($customer_id) {
		$sql = "SELECT count(*) as total FROM `oc_customer_weekly_veg_orders` cwo
				LEFT JOIN `oc_mpplan_customer` AS mc ON cwo.customer_id = mc.customer_id
				LEFT JOIN `oc_customer` AS c ON c.customer_id = mc.customer_id";
	...
	public function getVegOrderDetails($id) {
		$sql = "	SELECT vol.*,c.firstname,mc.plan_name,p.model,ds.name,wv.name as weight_value
					...
					LEFT JOIN `oc_delivery_settings` ds ON ds.id = vol.delivery_id
					LEFT JOIN `oc_weight_values` wv ON vol.weight_id = wv.id
					WHERE cwo.id = " . $id;
