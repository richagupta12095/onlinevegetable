...
class ModelSaleOrder extends Model {
	...
	public function getdlUser(){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user where user_group_id=13");
		return $query->rows;
	}

	public function assignorder($userId,$pid){
		$query = $this->db->query("update   " . DB_PREFIX . "order set deliverybid='".(int)$userId."' where order_id='".(int)$pid."'");
		return $query->row;
	}
	...
	public function getOrders($data = array()) {
		...
		$sql = "SELECT o.deliverybid,o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
	  ...
	  if (!empty($data['filter_deliverybid'])) {

			$sql .= " AND o.deliverybid = '" . (int)$data['filter_deliverybid'] . "'";
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_order_status'])) {
		  ...
		  if (!empty($data['filter_deliverybid'])) {
			...
			$sql .= " AND deliverybid = '" . (int)$data['filter_deliverybid'] . "'";
	...
	public function getOrdersexport($data = array()) {
		$sql = "SELECT  o.order_id, 
				   ...
				   LEFT JOIN " . DB_PREFIX . "order_status oos ON (o.order_status_id = oos.order_status_id) WHERE oos.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
   ...
   public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $pws_notify_by_sms = false, $override = false) {
	   // Update the DB with the new statuses
		...
		if ($order_info['order_status_id']==3) {
			$args_d0530['mobile_no']     = $order_info['telephone'];
			$args_d0530['message']       = "Order ID #".$order_id." has been Out for delivery,It will be delivered shortly.";
			$this->sendSms($args_d0530);
		}	else if($order_info['order_status_id']==5){			
				$args_d0530['mobile_no']     = $order_info['telephone'];
				$args_d0530['message']       = "Order ID #".$order_id." has been Deliverd Successfully";
				$this->sendSms($args_d0530);				
		}		
		return true;
	}
	...
	public function sendSms($data){
		...
		if(!empty($data) && isset($data['mobile_no'])){
			...
			curl_setopt_array($curl, array(
				...
				CURLOPT_HTTPHEADER => array(
					"authkey: 325067Ae3HWB2Y5e8215f4P1",
