...
class ControllerCommonColumnLeft extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
          ...
          //Opencart Bulk SMS
          $sms = $this->load->controller('extension/module/opc_sms/menu');
          ...
          if ($sms) {
			...
			//By Ankur Mittal
			if ($this->user->hasPermission('access', 'extension/module/bestproduct')) {		
				$catalog[] = array(
					'name'	   => $this->language->get('text_left_top_deal'),
					'href'     => $this->url->link('extension/module/bestproduct&module_id=36', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()		
				);					
			}
			//By Ankur Mittal
			if ($this->user->hasPermission('access', 'extension/total/order_discount')) {		
				$catalog[] = array(
			...
			/*CiBlog start*/
			$ciblog = $this->ciblog->getLeftMenu();
			if ($ciblog) {
				$this->load->language('extension/ciblog/cimenu');
				$data['menus'][] = array(
					'id'       => 'menu-ciblog',
					'icon'	   => 'fa-leaf',
					'name'	   => $this->language->get('text_ciblog_menu'),
					'href'     => '',
					'children' => $ciblog
				);
			}
