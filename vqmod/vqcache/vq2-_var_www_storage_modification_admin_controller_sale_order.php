...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
            ...
            $this->document->addScript('view/javascript/smshare/smshare-send-sms-from-order-list.js');

            

                $data['url_send_sms_from_order_list'] = $this->url->link('extension/module/smshare_core/sendsms', 'order_id=__ORDER_ID__&user_token=' . $this->session->data['user_token'], true);
		...
		if ($this->user->isLogged() && !empty($this->session->data['user_token'])) {
			$ugroupId=$this->user->getGroupId();
			if($ugroupId==13){
		...
		$data['export'] = $this->url->link('sale/order/export', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['orders'] = array();
		...
		$data['veg_orders'] = $this->url->link('customer/customer/vegOrdersList', 'user_token=' . $this->session->data['user_token'], true);
		...
		$dluser=$this->model_sale_order->getdlUser();
	...
	public function assigndb(){
			...
			if (isset($this->request->get['user_id'])) {
				$json['status']='error';	
				$json['message']='You Have select Delivery Body!  ';
			}
			...
			foreach($selected as $key => $value) {
					$assignorder=$this->model_sale_order->assignorder($userId,$value);
			}

			$json['status']='error';	
			$json['message']='Order item alloted to delivery boy!  ';
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['footer'] = $this->load->controller('common/footer');
                ...
                $this->load->model('extension/module/smshare_core_nomod');
                $data['pws_notify_by_sms_checkbox'] = $this->model_extension_module_smshare_core_nomod->get_my_checkbox($order_info, 'col-sm-2', 'col-sm-10');
                $data['pws_notify_by_sms_js_param'] = $this->model_extension_module_smshare_core_nomod->build_request_params_to_be_injected_in_the_url_of_the_order_status_update_ajax_request();
	...
	public function export() {
		...
		fputcsv($fp,  $orders_column);
		foreach($orders_list as $orders_row)
		{
			fputcsv($fp, $orders_row);
		}		
